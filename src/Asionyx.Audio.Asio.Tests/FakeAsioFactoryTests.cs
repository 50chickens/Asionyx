using Asionyx.Audio.Asio.Tests;
using NAudio.Wave;
...
namespace Asionyx.Audio.Asio.Tests
{
    ...
    public class FakeAsioFactoryTests
    {
        ...
        public void FakeInput_IsRoutedTo_Monitor_OnOutput34()
        {
            ...
            // Wait for monitor to receive at least one non-zero sample (reduce flakiness)
            var deadline = DateTime.UtcNow + TimeSpan.FromSeconds(2);
            bool receivedNonZero = false;
            while (DateTime.UtcNow < deadline)
            {
                if (!gotMonitor.WaitOne(TimeSpan.FromMilliseconds(250))) break;
                if (lastMonitorSamples.Length > 0)
                {
                    // compute simple absolute-sum over a short preview window
                    int preview = Math.Min(256, lastMonitorSamples.Length);
                    double energy = 0;
                    for (int i = 0; i < preview; i++) energy += Math.Abs(lastMonitorSamples[i]);
                    if (energy > 1e-6) { receivedNonZero = true; break; }
                }
                // continue waiting until deadline
            }
            Assert.That(receivedNonZero, Is.True, "Monitor did not receive non-zero samples in time");
            ...
            // Accept inverted polarity by using absolute correlation and keep the relaxed threshold
            Assert.That(Math.Abs(corr), Is.GreaterThan(0.1), $"Input/monitor correlation too low: {corr:F3}");
