...
# Ensure dotnet tools are on PATH for this session (needed for reportgenerator)
$env:PATH += ";$env:USERPROFILE\.dotnet\tools"

# Ensure local .config/dotnet-tools.json exists for local tool install
$localToolManifest = Join-Path $PSScriptRoot ".config/dotnet-tools.json"
if (-not (Test-Path (Split-Path $localToolManifest))) { New-Item -ItemType Directory -Path (Split-Path $localToolManifest) | Out-Null }
...
function Test-Integration {
    ...
    param(
        ...
        if ($covFiles.Count -gt 0) {
            $covFilesList = $covFiles | ForEach-Object { $_.FullName } | Join-String -Separator ";"
            Write-Host "\n==== Generating HTML coverage report with ReportGenerator ====" -ForegroundColor Yellow
            ...
            if (Test-Path (Join-Path $reportDir 'index.html')) {
                ...
                if (Test-Path $summaryFile) {
                    Write-Host "\n==== COVERAGE SUMMARY ====" -ForegroundColor Cyan
